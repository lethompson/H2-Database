...
http://localhost:8082 (URL H2 Database)
...
#Query 1: 
Retrieve the First name and Last name of the managers who have at least one dependent. The first name
should be printed as Name and Last name as Last as well as include the name
of the dependent in your result.

SELECT FNAME AS NAME, LNAME AS LAST, DEP.DEPENDENT_NAME FROM employee AS EMP, dependent AS DEP WHERE EMP.SUPER_SSN = DEP.ESSN;


#Query 2:
For every project located in Stafford, list the name of the project and the
department and last name of the department manager that controls these projects


SELECT PRO.PNAME AS project_name, DEPT.DNAME AS department_name, DEPT_LOC.DLOCATION AS dept_location, EMP.LNAME AS manager_lname FROM employee AS EMP, department AS DEPT, dept_locations AS DEPT_LOC, project AS PRO WHERE (EMP.DNO = DEPT.DNUMBER AND EMP.DNO=DEPT_LOC.DNUMBER) AND DEPT.DNUMBER=4;


# Lab 2:

Query #1: For every department located in Houston
...
SELECT DEPT.DNAME AS department_name, EMP.LNAME AS manager_lname , DEPT_LOC.DLOCATION AS dept_location, PRO.PNAME AS project_name  FROM employee AS EMP, department AS DEPT, dept_locations AS DEPT_LOC, project AS PRO WHERE (EMP.DNO = DEPT.DNUMBER AND EMP.DNO=DEPT_LOC.DNUMBER) AND DEPT.DNUMBER=1;


Query #2: Retrieve the First and Last name of all managers who work on the project that
Are controlled by the department that he/she manages
